...
if(nrow(params$atfm) > 0) {
  ...
  atfm_pa %>% plot_ly( x = ~YEAR, 
                       ...
                       colors = atfm_col, 
                       type = "bar") %>%
    add_annotations(data = atfm_pa %>% select(YEAR, AVG_ARR_ATFM_DLY) %>% unique(),
                    x = ~YEAR,
...
if(nrow(params$atfm) > 0) {
  ...
  MONTHLY_HIGH_ATFM <- atfm_pm %>%
    ...
    mutate(PCT_REG = round((AVG_ARR_ATFM_REG / AVG_ARR_ATFM_DLY)*100 ,2))%>%
    select(YEAR, MONTH_NUM, AVG_ARR_ATFM_DLY, PCT_REG, REG_REASON)%>%
    mutate(KEY_MSG = paste(round(AVG_ARR_ATFM_DLY,2),"min/arr <br>(",round(PCT_REG),"% due to",REG_REASON,")"))%>%
    select(YEAR, MONTH_NUM, KEY_MSG)
  
  #atfm_pm <-left_join(atfm_pm, MONTHLY_HIGH_ATFM)
...
if(nrow(params$atfm) > 0) {
  ...
  {
    key_msg <- paste0("<b>Latest worst day in last 12 months: "   , worst_day_date,
...
SLOT_YY_PLOT1 <- params$slot_yy %>%
  select( AIRPORT,
          ...
          TOT_FLT_DEP_REG_1)  %>% 
  mutate(PCT_DEP_REG = (TOT_FLT_DEP_REG_1/TOT_FLT_DEP_1)*100,
...
SLOT_YY_PLOT2 <- params$slot_yy %>% 
  ...
  tidyr::pivot_longer(cols      = c(TOT_FLT_DEP_OUT_LATE_1, TOT_FLT_DEP_OUT_EARLY_1, TOT_FLT_DEP_IN_1),
                      names_to  = "SLOT_CAT",
                      values_to = "NB_SLOT")

slot_col <- c("#BDD7EE", #blue
              "#C6E0B4", #green
              "#F8CBAD") #orange

SLOT_YY_PLOT2$SLOT_CAT <- factor(SLOT_YY_PLOT2$SLOT_CAT,
                                 levels=c( "TOT_FLT_DEP_OUT_EARLY_1", 
                                           "TOT_FLT_DEP_IN_1", 
                                           "TOT_FLT_DEP_OUT_LATE_1"),
                                 labels=c( "Before the STW<br>(Early Take-Off Traffic)",
...
SLOT_YY_PLOT2 %>%
  ...
  layout( barmode   = "stack",
          hovermode = "x unified",
          xaxis     = list(title = ""),
          yaxis     = list( title = "Number of Regulated departures",
                            titlefont = list(size = 11))
          ##############################################################################
          ,annotations = list(
            x         = 1, 
...
SLOT_MM_PLOT2 <- params$slot_mm %>% 
  select(YEAR, MONTH_NUM, TOT_FLT_DEP_OUT_LATE_1, TOT_FLT_DEP_OUT_EARLY_1, TOT_FLT_DEP_IN_1 )  %>%
  tidyr::pivot_longer(cols      = c(TOT_FLT_DEP_OUT_LATE_1, TOT_FLT_DEP_OUT_EARLY_1, TOT_FLT_DEP_IN_1),
                      names_to  = "SLOT_CAT",
                      values_to = "NB_SLOT")
...
if (nrow(params$cdo_cco)>0)
{
  ...
  cat("Analysis on vertical flight efficiency during climb and descent is not available. Please contact pru-support@eurocontrol.int for further information.")
  
  # To add clickable link to mail address: add mail address in params in yaml header and use it in cat("Analysis on vertical flight efficiency during climb and descent is not available. Please contact [", pru-support@eurocontrol.int, "](", mailto:pru-support@eurocontrol.int, ") for further information.") - See also https://stackoverflow.com/questions/52745926/r-markdown-hyperlink-outside-rmd-file
...
```{r Med-CDO-CCO-alt-avg-time-lvl}
if (nrow(params$cdo_cco)>0)
{
...
### Annual Departure Punctuality

```{r Punctuality DEP YEARLY LAYOUT}
if (params$apdf == 'Y')
{ 
  
  pun_col <- c( "#1f4e79", # EARLY >30		#rgb 31,78,121	
  ...
  punc_dep_yy_plot %>% plot_ly( x      = ~YEAR, 
                                y      = ~AVG_PER_CATEG,
                                type   = "bar",
                                color  = ~PUNCT_CAT, 
                                colors = pun_col
    ...
    layout(barmode = "stack", 
           hovermode = "x unified",
           xaxis = list(title = ""),
           yaxis = list(title="Departure Punctuality repartition (%)",
                        titlefont = list(size = 11),
                        hoverformat = ".2f")
           ##############################################################################
           ,annotations = list(
  ...
  cat("
Analysis on punctuality are only available for airports providing the required data 
...
```{r Punctuality DEP MONHTLY LAYOUT}
...
{ 
  ...
  punc_dep_mm_plot$PUNCT_CAT <- factor(punc_dep_mm_plot$PUNCT_CAT,
                                       levels= c("Early >30","Early 16-30","Early 5-15","On Time -+4","Late 5-15","Late 16-30","Late 31-60","Late >60"))
...
### Annual Arrival Punctuality

```{r Punctuality ARR YEARLY LAYOUT}
if (params$apdf == 'Y')
{ 
  
  punc_arr_yy_plot <- params$punc_arr_yy
  
  punc_arr_yy_plot$PUNCT_CAT <- factor(punc_arr_yy_plot$PUNCT_CAT,
                                       levels= c("Early >30","Early 16-30","Early 5-15","On Time -+4","Late 5-15","Late 16-30","Late 31-60","Late >60"))
  
  
  punc_arr_yy_plot %>% plot_ly( x = ~YEAR, 
                                y = ~AVG_PER_CATEG,
                                color = ~PUNCT_CAT, 
                                colors = pun_col, 
                                type = "bar") %>%
    layout(barmode = "stack", 
           hovermode = "x unified",
           xaxis = list(title = ""),
           yaxis = list(title="Arrival Punctuality repartition (%)",
                        titlefont = list(size = 11),
                        hoverformat = ".2f")
           ##############################################################################
           ,annotations = list(
...
### Monthly Arrival Punctuality

```{r Punctuality ARR MONHTLY LAYOUT}
...
{ 
  ...
  punc_arr_mm_plot %>%plot_ly(x = ~MONTH_NUM, 
                              y = ~AVG_PER_CATEG,
                              color = ~PUNCT_CAT, 
                              colors = pun_col, 
                              type = "bar",
...
{ 
  ...
  ASMA_VAR <- params$asma_mm %>% 
    ...
    group_by(YEAR) %>%
    summarise(TOT_ADD_TIME = sum(TOT_ADD_TIME, na.rm = TRUE),
  ...
  #               CUM_ASMA$YEAR[1],
  #               ":<br>", 
  #               round(100* CUM_ASMA[2,"CHANGE"], 2),"% </b>")
  ...
  key_msg <- list(x         = unique(max(ASMA_VAR$YEAR)),
                  y         = msg_y,
                  text      = msg,
                  size      = 14,
                  showarrow = FALSE)
...
{ 
  ...
  key_msg <- list(x         = unique(max(TXOT_VAR$YEAR)),
                  y         = msg_y,
                  text      = msg,
                  size      = 14,
                  showarrow = FALSE)
  ...
  cat("
Analysis on taxi-out times are only available for airports providing the required data 
using the Airport Operator Data Flow (APDF).
...
{ 
  ...
  key_msg <- list(x         = unique(max(TXIN_VAR$YEAR)),
                  y         = msg_y,
                  text      = msg,
                  size      = 14,
                  showarrow = FALSE)
  ...
  cat("
Analysis on taxi-in times are only available for airports providing the required data 
using the Airport Operator Data Flow (APDF).
...
{ 
  ...
  predep_col <- c("#F8CBAD", #RGB 248,203,173 # Unidentified
                  "#BDD7EE", #RGB 189,215,238 # Other reasons
                  "#4682B4") #RGB 198,224,180 # ATC Pre-Departure delay (code 89)
...
{ 
  ...
  params$pddly_yy_avg %>% plot_ly(x = ~YEAR, 
                                  y = ~AVG_PREDEP_DLY,
                                  type = "bar",
                                  color = I("steelblue")) %>%
    layout(  xaxis = list(title=""),
...
{ 
  ...
  turn_col <- c("#C6E0B4", #RGB 198,224,180 # Heavy
                "#F8CBAD", #RGB 248,203,173 # Medium Jet
                "#BDD7EE") #RGB 189,215,238 # Medium Turboprop
  ...
  TURN_YY %>%
    ...
    layout( barmode   = "group",
            hovermode = "x unified",
            xaxis     = list(title = ""),
            yaxis     = list( title = "Average Actual Turnaround time [min/flight]",
                              titlefont = list(size = 11),
                              hoverformat = ".2f")
            ##############################################################################
            ,annotations = list(
...
{ 
  TURN_MM <- params$turn_mm %>% 
    ...
    mutate( LBL_ACTUAL     = paste0("Actual Turnaround ", AC_CLASS),
            ...
            HOVERTEXTACTT  = paste0("Class: ", AC_CLASS,
                                    "<br>Avg. Turn:", round(AVG_ACTT,2),"min",
                                    "<br>Nb Turn. :", TOT_TURN),
            HOVERTEXTSDTT  = paste0("Class: ", AC_CLASS,
  ...
  TURN_MM %>%  plot_ly() %>% 
    ...
    layout( updatemenus = list( button_type_list ),
            hovermode   = "x unified",
            xaxis       = tick_yr_no_title,
            yaxis       = list(title = "Average Turnaround times [min/flight]",
                               titlefont = list(size = 11))
            ##############################################################################
            ,annotations = list(
              x         = 1, 
