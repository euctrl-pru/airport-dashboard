...
annotations = vector(mode = "list", length = length(filter_years))
for (i in 1:nrow(MONTHLY_HIGH_TXOT)) {
  annotation <- list(x = MONTHLY_HIGH_TXOT$MONTH_NUM[i],
                     y = MONTHLY_HIGH_TXOT$AVG_TIME[i],
                     xref='x',
                     yref='y',
                     text = paste0(
                       "<b>Highest average<br>additional Taxi-Out time:<br>",
                       round(MONTHLY_HIGH_TXOT$TIME[i], 2), " min/arr</b>"),
                     yshift=40,
                     xshift = ifelse(MONTHLY_HIGH_TXOT$MONTH_NUM[i]==1,
                                     55,
                                     ifelse(MONTHLY_HIGH_TXOT$MONTH_NUM[i]==12, -55, 0)),
                     align = ifelse(MONTHLY_HIGH_TXOT$MONTH_NUM[i]==1,
                                    "left",
                                    ifelse(MONTHLY_HIGH_TXOT$MONTH_NUM[i]==12, "right", "center")),
                     showarrow = FALSE)
  annotations[[i]] <- annotation
}
