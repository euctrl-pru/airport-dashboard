...
```{r TAXI-IN MONTHLY LAYOUT, results='asis', eval=params$apdf == "Y", echo = FALSE}
  ...
  annotations = vector(mode = "list", length = length(filter_years))
  for (i in 1:nrow(MONTHLY_HIGH_TXIN)) {
    annotation <- list(x = MONTHLY_HIGH_TXIN$MONTH_NUM[i],
                       y = MONTHLY_HIGH_TXIN$AVG_TIME[i],
                       xref='x',
                       yref='y',
                       text = paste0(
                         "<b>Highest average<br>additional Taxi-In time:<br>",
                         round(MONTHLY_HIGH_TXIN$TIME[i], 2), " min/arr</b>"),
                       yshift=40,
                       xshift = ifelse(MONTHLY_HIGH_TXIN$MONTH_NUM[i]==1,
                                       55,
                                       ifelse(MONTHLY_HIGH_TXIN$MONTH_NUM[i]==12, -55, 0)),
                       align = ifelse(MONTHLY_HIGH_TXIN$MONTH_NUM[i]==1,
                                      "left",
                                      ifelse(MONTHLY_HIGH_TXIN$MONTH_NUM[i]==12, "right", "center")),
                       showarrow = FALSE)
    annotations[[i]] <- annotation
  }
