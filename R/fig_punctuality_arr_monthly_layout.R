...
punc_arr_mm_plot %>%
  ...
  add_download_button(
    punc_arr_mm_plot %>% 
    select(AIRPORT, PHASE, YEAR, PUNCT_CAT, PUNC_CAT_SHARE = AVG_PER_CATEG) , "PUNCT_ARR_MM")




# Factsheet figure
punc_arr_mm_plot_curr_year=filter(punc_arr_mm_plot, YEAR == max_year) %>% 
  mutate(Month=factor(month.abb[MONTH_NUM], levels = month.abb))

punc_arr_mm_plot_fig = ggplot(data=punc_arr_mm_plot_curr_year) +
...
ggsave(here("media", "factsheet", paste0("Punc_Arr_Monthly_", params$icao, ".pdf")), plot=punc_arr_mm_plot_fig, 
       width = Punctuality_layout2[2]*Page_width, height = Punctuality_height2, units = "cm", dpi=100, limitsize = FALSE)
