...
for (i in 1:length(filter_years)) {
  ...
  if (nrow(MONTHLY_HIGH_TXOT_temp)>0) {
    annotation <- list(x = MONTHLY_HIGH_TXOT_temp$MONTH_NUM,
                       y = MONTHLY_HIGH_TXOT_temp$AVG_TIME,
                       xref='x',
                       yref='y',
                       text = paste0(
                         "<b>Highest average<br>additional Taxi-Out time:<br>",
                         round(MONTHLY_HIGH_TXOT_temp$TIME, 2), " min/arr</b>"),
                       yshift=40,
                       xshift = ifelse(MONTHLY_HIGH_TXOT_temp$MONTH_NUM==1,
                                       55,
                                       ifelse(MONTHLY_HIGH_TXOT_temp$MONTH_NUM==12, -55, 0)),
                       align = ifelse(MONTHLY_HIGH_TXOT_temp$MONTH_NUM==1,
                                      "left",
                                      ifelse(MONTHLY_HIGH_TXOT_temp$MONTH_NUM==12, "right", "center")),
                       showarrow = FALSE)
  } else {
    annotation <- list(x = 6,
                       y = 1,
...
layout(
  ...
  add_download_button(TXOT_MM)


if (!is.null(annotations[[length(filter_years)]])) {
  txot_mm_fig=txot_mm_fig %>% 
...
ggsave(here("media", "factsheet", paste0("Taxi_Out_Times_Monthly_", params$icao, ".pdf")), plot=TXOT_MM_fig, 
