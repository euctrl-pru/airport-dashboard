...
```{r Avg-time-lvl-plotly, results='asis', eval=nrow(params$cdo_cco)>0, echo = FALSE}
...
```{r Avg-time-lvl-ggplot, include=FALSE}
...
AVG_TIME_LVL_PLOT_curr_year=filter(AVG_TIME_LVL_PLOT, YEAR==max_year) %>% 
  ...
  mutate(Month=factor(month.abb[MONTH_NUM], levels = month.abb),
         Metric=factor(Metric, levels = unique(AVG_TIME_LVL_PLOT_curr_year$Metric))) %>% 
  arrange(YEAR, Month, Metric)


AVG_TIME_LVL_PLOT_fig = ggplot(data=AVG_TIME_LVL_PLOT_curr_year) +
  geom_line(aes(x=Month, y = Value/60, group=Metric, colour=Metric, linetype=Metric), size=2) +
  ...
  labs(x="", y="", title="Average time flown level per flight (min.)")

ggsave(here("R", "Factsheet", "Figures", paste0("Avg_time_lvl_", Airport, ".png")), 
