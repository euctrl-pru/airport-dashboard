...
```{r Med-CDO-CCO-alt-plotly, results='asis', eval=nrow(params$cdo_cco)>0, echo = FALSE}
...
```{r Med-CDO-CCO-alt-ggplot, include=FALSE}
...
MED_CDO_CCO_ALT_PLOT_curr_year=filter(MED_CDO_CCO_ALT_PLOT, YEAR==max_year) %>% 
  ...
  mutate(Month=factor(month.abb[MONTH_NUM], levels = month.abb),
         Metric=factor(Metric, levels = unique(MED_CDO_CCO_ALT_PLOT_curr_year$Metric))) %>% 
  arrange(YEAR, Month, Metric)


MED_CDO_CCO_ALT_PLOT_fig = ggplot(data=MED_CDO_CCO_ALT_PLOT_curr_year) +
  geom_line(aes(x=Month, y = Value/60, group=Metric, colour=Metric), size=2) +
  ...
  labs(x="", y="", title="Median CDO/CCO altitude (feet)")
